	...
	<div class="card card-outline card-primary">
		...
		<div class="card-body">
			<table style="width: 100%">
				<tr>
					<th>Client ID#</th>
					<td><?php if ($row['IdentityType'] == '1') {
							echo 'ID TYPE: NRC - ' . $row['ClientID'];
						} elseif ($row['IdentityType'] == '2') {
							echo 'ID Type: PASSPORT - ' . $row['ClientID'];
						} elseif ($row['IdentityType'] == '3') {
							echo 'ID Type: = DRIVER\'\S LICENSE - ' . $row['ClientID'];
						} elseif ($row['IdentityType'] == '4') {
							echo 'ID Type: VOTER\'\S VOTER - ' . $row['ClientID'];
						} else {
							echo 'ID Type: PACRA/COMPANY - ' . $row['ClientID'];
						} ?></td>
				</tr>
				<tr>
					<th>Client Type</th>
